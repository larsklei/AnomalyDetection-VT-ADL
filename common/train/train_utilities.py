...
from custom_model.visiontransformer_layers import VisionTransformerEncoder, get_decoder
...
class WarmupScheduler(keras.optimizers.schedules.LearningRateSchedule):
	...
	def get_config(self):
		if isinstance(self.after_warmup_lr_sched,
			...
			config = {
				"after_warmup_lr_sched": keras.saving.serialize_keras_object(self.after_warmup_lr_sched),
	...
	def from_config(cls, config):
		if isinstance(self.after_warmup_lr_sched,
		              keras.optimizers.schedules.LearningRateSchedule):
			lr_schedule_config = config.pop("after_warmup_lr_sched")
			warm_up_schedule = keras.saving.deserialize_keras_object(lr_schedule_config)
			return cls(warm_up_schedule, **config)
		else:
			return cls(**config)
...
def create_model(
	...
	for unit in encoder_hidden_units:
		encoder.add(VisionTransformerEncoder(
			num_heads=num_heads,
			key_dim=latent_dim,
			hidden_units=unit
		))
